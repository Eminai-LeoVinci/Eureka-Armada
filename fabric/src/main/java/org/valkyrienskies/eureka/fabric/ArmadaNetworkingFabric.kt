...
object ArmadaNetworkingFabric {
    ...
    private val BONDS_RL: ResourceLocation = ResourceLocation(EurekaMod.MOD_ID, "armada_bonds")
    ...
    fun registerClient() {
        ClientPlayNetworking.registerGlobalReceiver(BONDS_RL) { client, _, rawBuf, _ ->
            val data = rawBuf.readByteArray()
            val snapshot = decode(data)
            client.execute { ArmadaClientBonds.update(snapshot) }
        }
    }
    ...
    fun broadcastBonds(level: ServerLevel) {
        ...
        val data = encode(entries)
        for (player in level.players()) ServerPlayNetworking.send(player, BONDS_RL, wrap(data))
    }
