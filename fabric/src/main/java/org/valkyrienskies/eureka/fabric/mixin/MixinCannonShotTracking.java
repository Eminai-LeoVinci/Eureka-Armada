...
public abstract class MixinCannonShotTracking {
    ...
    private boolean vs_eureka$trackCannonballsBeyondSentChunks(
        final ChunkMap instance, final ServerPlayer player, final int chunkX, final int chunkZ,
        final Operation<Boolean> original
    ) {
        if (this.entity instanceof CannonShot) return true;
        return original.call(instance, player, chunkX, chunkZ);
    }
}
