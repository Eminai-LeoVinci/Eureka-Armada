...
class CannonBlockEntity(pos: BlockPos, state: BlockState) :
    ...
    // Plain helpers on 1.20.1 (BaseContainerBlockEntity only grew the abstract pair in 1.20.5).
    ...
    override fun saveAdditional(output: CompoundTag) {
        ...
        if (!powderA.isEmpty) output.put("Powder", powderA.save(CompoundTag()))
        if (!powderB.isEmpty) output.put("Powder2", powderB.save(CompoundTag()))
        if (!powderC.isEmpty) output.put("Powder3", powderC.save(CompoundTag()))
        if (!shot.isEmpty) output.put("Shot", shot.save(CompoundTag()))
    ...
    override fun load(input: CompoundTag) {
        super.load(input)
        powderA = ItemStack.of(input.getCompound("Powder"))
        powderB = ItemStack.of(input.getCompound("Powder2"))
        powderC = ItemStack.of(input.getCompound("Powder3"))
        shot = ItemStack.of(input.getCompound("Shot"))
    ...
    fun load(stack: ItemStack, consume: Boolean): Int {
        ...
        for (slot in slots) {
            ...
            val room = when {
                ...
                ItemStack.isSameItemSameTags(current, stack) -> MAGAZINE_CAPACITY - current.count
                else -> 0
            }
            if (room <= 0) continue
