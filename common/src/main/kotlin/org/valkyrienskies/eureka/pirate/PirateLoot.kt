...
object PirateLoot {
    ...
    private fun stacksOf(level: ServerLevel, id: String, entry: LootEntry): List<ItemStack> {
        val item = BuiltInRegistries.ITEM.getOptional(ResourceLocation(id)).orElse(null)
        if (item == null) {
    ...
    private fun enchant(level: ServerLevel, stack: ItemStack, entry: LootEntry): ItemStack {
        ...
        if (entry.enchantRandomly) {
            ...
            // (1.20.1's form takes a treasure flag where 1.21.1 takes a registry tag.)
            out = EnchantmentHelper.enchantItem(level.random, out, power, entry.treasureAllowed)
        }
        val fixed = entry.enchantments
        if (!fixed.isNullOrEmpty()) {
            for (spec in fixed) {
                val ench = BuiltInRegistries.ENCHANTMENT.getOptional(ResourceLocation(spec.id)).orElse(null) ?: continue
                val lvl = spec.level.coerceAtLeast(1)
                // A fixed enchant on a plain book means an enchanted book, same as the random roll gives.
                if (out.`is`(Items.BOOK)) out = ItemStack(Items.ENCHANTED_BOOK, out.count)
                if (out.`is`(Items.ENCHANTED_BOOK)) {
                    EnchantedBookItem.addEnchantment(out, EnchantmentInstance(ench, lvl))
                } else {
                    out.enchant(ench, lvl)
                }
            }
        }
        return out
