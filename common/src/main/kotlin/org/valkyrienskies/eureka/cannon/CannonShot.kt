...
class CannonShot(type: EntityType<out CannonShot>, level: Level) : Entity(type, level), ItemSupplier {
    ...
    override fun defineSynchedData() {
        ...
        entityData.define(POWDER, PowderCharge.SINGLE.ordinal)
        entityData.define(GRAVITY, PowderCharge.SINGLE.gravity.toFloat())
        entityData.define(DRAG, PowderCharge.SINGLE.drag.toFloat())
    }
    ...
    override fun readAdditionalSaveData(input: CompoundTag) {
        ...
        entityData.set(GRAVITY, charge.gravity.toFloat())
        entityData.set(DRAG, charge.drag.toFloat())
        entityData.set(SHOWN, ItemStack.of(input.getCompound("Shown")))
    ...
    override fun addAdditionalSaveData(output: CompoundTag) {
        ...
        if (!item.isEmpty) output.put("Shown", item.save(CompoundTag()))
    }
