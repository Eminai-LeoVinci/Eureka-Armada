...
object CrewRoll {
     ...
     * One crew, as a row.
     *
     * [aboard] marks the crew this wheel already KEEPS, which is the one that is free to call. It is a
     * fact about the ARTICLES, not about the deck: a binding outlives disassembly, bottling and a relog
     * on purpose, so a wheel goes on naming its crew while every one of them is missing.
     *
     * [present] is the other half, and the one the Summon button needs: how many of this crew are
     * actually FOUND standing on this hull right now. The button used to grey out whenever [aboard] was
     * set, which meant the wheel that still remembered a crew refused to call them back -- exactly the
     * case where calling them is the whole point.
     *
     * [fare] is what calling this crew would cost right now,
    ...
    fun build(captain: ServerPlayer, station: ShipHelmBlockEntity): Roll {
        ...
        // of an armada reads as present at the flagship's wheel, exactly as gunnery and the roster treat it.
        ...
        val entries = ledger.crewsOf(captain.uuid).map { crew ->
            ...
            // at all (dead, despawned, removed by command) or who is found somewhere else is NOT present,
            // which is the whole distinction the Summon button turns on.
