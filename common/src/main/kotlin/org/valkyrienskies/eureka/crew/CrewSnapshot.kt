...
object CrewSnapshot {
    ...
    fun capture(villager: Villager): CompoundTag? =
        try {
            val output = TagValueOutput.createWithContext(ProblemReporter.DISCARDING, villager.registryAccess())
            villager.saveWithoutId(output)
            output.buildResult().also { writePapers(it, villager) }
        } catch (ex: Exception) {
            null
        }
    ...
    fun papers(tag: CompoundTag?): Papers? {
        val papers = tag?.getCompound(PAPERS_KEY)?.orElse(null) ?: return null
        return Papers(
            profession = papers.getString(PROFESSION_KEY).orElse(NO_PROFESSION),
            type = papers.getString(TYPE_KEY).orElse(DEFAULT_TYPE),
            level = papers.getInt(LEVEL_KEY).orElse(0),
            xp = papers.getInt(XP_KEY).orElse(0)
        )
    }

    private fun <T : Any> idOf(holder: Holder<T>, fallback: String): String =
    ...
    fun restore(level: ServerLevel, tag: CompoundTag, at: Vec3): Villager? =
        try {
            val villager = EntityType.VILLAGER.create(level, EntitySpawnReason.LOAD)
            if (villager == null) null else {
                villager.load(TagValueInput.create(ProblemReporter.DISCARDING, level.registryAccess(), tag))
                villager.uuid = UUID.randomUUID()
                villager.setPos(at.x, at.y, at.z)
                villager.deltaMovement = Vec3.ZERO
                villager.fallDistance = 0.0
