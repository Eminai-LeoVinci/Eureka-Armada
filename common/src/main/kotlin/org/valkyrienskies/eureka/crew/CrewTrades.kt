...
object CrewTrades {
    ...
    private fun offerOf(entry: EurekaLootConfig.TradeEntry, rng: RandomSource): MerchantOffer {
        ...
        return when (entry.type) {
            "buy" -> MerchantOffer(
                ItemStack(itemOf(entry.item)!!, count), ItemStack(Items.EMERALD, emeralds),
                uses, xp, PRICE_MULTIPLIER
            )
            "balloon" -> MerchantOffer(
                ItemStack(Items.EMERALD, emeralds), ItemStack(rollBalloon(rng)),
                uses, xp, PRICE_MULTIPLIER
            )
            else -> MerchantOffer(
                ItemStack(Items.EMERALD, emeralds), ItemStack(itemOf(entry.item)!!, count),
                uses, xp, PRICE_MULTIPLIER
            )
        }
    }

    /** The one line that differs between the trees: 1.20.1 has no parse(), so the constructor stands in. */
