 ...
 *
 * The listings live here in :common because `VillagerTrades.ItemListing`, `MerchantOffer` and `ItemCost` are
 * all vanilla. Only the registration call needs Fabric API, and that is in `CrewRegistrationsFabric`.
 *
...
object CrewTrades {
    ...
    private class Sell(
        ...
        override fun getOffer(trader: Entity, rng: RandomSource): MerchantOffer =
            MerchantOffer(ItemCost(Items.EMERALD, emeralds), result(), maxUses, xp, PRICE_MULTIPLIER)
    }
    ...
    private class Buy(
        ...
        override fun getOffer(trader: Entity, rng: RandomSource): MerchantOffer =
            MerchantOffer(ItemCost(item, count), ItemStack(Items.EMERALD, emeralds), maxUses, xp, PRICE_MULTIPLIER)
    }
    ...
    private class BalloonForEmeralds(
        ...
        override fun getOffer(trader: Entity, rng: RandomSource): MerchantOffer {
            ...
            return MerchantOffer(
                ItemCost(Items.EMERALD, emeralds), ItemStack(block), maxUses, xp, PRICE_MULTIPLIER
            )
        }
    }
