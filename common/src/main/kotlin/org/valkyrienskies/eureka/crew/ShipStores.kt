...
object ShipStores {
    ...
    class Manifest(val level: ServerLevel, val holds: List<Hold>) {
        ...
        fun count(predicate: (ItemStack) -> Boolean): Int {
            ...
            for (hold in holds) {
                ...
                for (slot in 0 until box.containerSize) {
                    ...
                    val packed = packedItems(stack) ?: continue
                    for (inner in packed) {
                        if (!inner.isEmpty && predicate(inner)) total += inner.count
                    }
                }
            }
            return total
            ...
            for (hold in visit) {
                ...
                val candidates = (0 until box.containerSize)
                    .mapNotNull { slot ->
                        ...
                        val packed = packedItems(item) ?: return@mapNotNull null
                        val items = packed.map { it.copy() }
                        // A shulker holding something else entirely is somebody's filing; leave it be.
                        val holdsOther = items.any { !it.isEmpty && !ItemStack.isSameItemSameTags(it, stack) }
                ...
                for ((slot, boxStack, items) in candidates) {
                    ...
                    for (i in room.indices) {
                        ...
                        if (there.isEmpty) {
                            ...
                            moved += move
                        } else if (ItemStack.isSameItemSameTags(there, stack) && there.count < cap) {
        ...
        private fun unpack(
            ...
            for (hold in visit) {
                ...
                val slots = (0 until box.containerSize)
                    .filter { slot ->
                        val stack = box.getItem(slot)
                        !stack.isEmpty && !predicate(stack) && packedItems(stack) != null
                    }
                ...
                for (slot in slots) {
                    ...
                    val packed = packedItems(packedBox) ?: continue
        ...
        private fun packedCount(box: ItemStack, predicate: (ItemStack) -> Boolean): Int {
            val packed = packedItems(box) ?: return 0
            var total = 0
            for (inner in packed) if (!inner.isEmpty && predicate(inner)) total += inner.count
            return total
        }
        ...
        fun deposit(tag: HoldTag?, stack: ItemStack): Deposit {
            ...
            for (hold in visit) {
                ...
                for (slot in 0 until box.containerSize) {
                    ...
                    if (there.isEmpty || !ItemStack.isSameItemSameTags(there, stack)) continue
    ...
    fun tally(manifest: Manifest): Stores {
        ...
        fun record(stack: ItemStack): Boolean {
            ...
            val burn = FuelRegistry.INSTANCE.get(stack)
            if (burn > 0) {
        ...
        for (hold in manifest.holds) {
            ...
            for (slot in 0 until box.containerSize) {
                ...
                val packed = packedItems(stack) ?: continue
                for (inner in packed) if (!inner.isEmpty) record(inner)
            }
        }
...
// 1.20.1: a shulker's contents live in the item's BlockEntityTag rather than a data component. These two
// are the one shared read/write for that shelf -- ShipStores' restocks, HoldTags' categorising, and the
// shipwright's salvage packing all go through them so the layout is written one way only.
// Read the full 27-slot shelf (empty slots included, so writes keep their slot layout), or null for an
// item that is not carrying container data at all.
internal fun packedItems(stack: ItemStack): List<ItemStack>? {
    val tag = BlockItem.getBlockEntityData(stack) ?: return null
    if (!tag.contains("Items", 9)) return null
    val list = NonNullList.withSize(ShipStores.SHULKER_SLOTS, ItemStack.EMPTY)
    ContainerHelper.loadAllItems(tag, list)
    return list
}

internal fun writePacked(stack: ItemStack, items: List<ItemStack>) {
    val list = NonNullList.withSize(ShipStores.SHULKER_SLOTS, ItemStack.EMPTY)
    for (i in items.indices) if (i < ShipStores.SHULKER_SLOTS) list[i] = items[i]
    val tag = BlockItem.getBlockEntityData(stack) ?: CompoundTag()
