...
object GunStations {
    ...
    fun unseat(level: ServerLevel, villagerId: UUID) {
        seats.remove(villagerId)?.seat?.takeIf { !it.isRemoved }?.let { wake(it); it.kill() }
    ...
    private fun ensureMounted(level: ServerLevel, villager: Villager, gunPos: BlockPos): Boolean {
        ...
        // (1.20.1 has no attachment points; the villager's riding offset is the same lever there.)
        val rideY = villager.myRidingOffset
