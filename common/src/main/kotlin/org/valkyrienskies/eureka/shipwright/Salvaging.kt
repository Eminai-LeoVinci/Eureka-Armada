 ...
 * into one. Boxes are written through their [DataComponents.CONTAINER] component, in place, so colour,
...
object Salvaging {
    ...
    private fun hand(
        ...
        while (true) {
            ...
            val offered = minOf(left, item.defaultMaxStackSize)
    ...
    private fun packBoxes(
        ...
        for (slot in 0 until player.inventory.containerSize) {
            ...
            if (box.count != 1 || !box.`is`(ItemTags.SHULKER_BOXES)) continue
            // An empty box and a box with no contents component are the same thing; both are fair game.
            if (box.get(DataComponents.CONTAINER)?.stream()?.anyMatch { !it.isEmpty } == true) continue
