...
class ShipwrightLedger : SavedData() {
    ...
    companion object {
        ...
        private val TYPE: SavedData.Factory<ShipwrightLedger> = SavedData.Factory(
            { ShipwrightLedger() },
            { tag, _ -> load(tag) },
            DataFixTypes.LEVEL
        )
        ...
        fun get(server: MinecraftServer): ShipwrightLedger {
            ...
            return server.overworld().dataStorage.computeIfAbsent(TYPE, SAVED_DATA_ID)
        }
        ...
        fun load(tag: CompoundTag): ShipwrightLedger {
            ...
            val list = tag.getListOpt(LIBRARIES_KEY).orElse(ListTag())
            for (element in list) {
                val entry = element as? CompoundTag ?: continue
                val owner = parseUuid(entry.getStringOpt(OWNER_KEY).orElse("")) ?: continue
                val library = Library(entry.getIntOr(SLOTS_KEY, EurekaConfig.SERVER.shipwrightSlotsStart))

                val shelf = entry.getListOpt(PLANS_KEY).orElse(ListTag())
                for (i in 0 until shelf.size) {
                    val row = shelf.getCompoundOpt(i).orElse(null) ?: continue
                    val shipName = row.getStringOpt(SHIP_NAME_KEY).orElse("")
                    val template = row.getStringOpt(TEMPLATE_KEY).orElse("")
                ...
                val jobs = entry.getListOpt(REPAIRS_KEY).orElse(ListTag())
                for (i in 0 until jobs.size) {
                    val row = jobs.getCompoundOpt(i).orElse(null) ?: continue
                    val slug = row.getStringOpt(SHIP_SLUG_KEY).orElse("")
                    val plansName = row.getStringOpt(PLANS_NAME_KEY).orElse("")
                ...
                val piles = entry.getListOpt(SALVAGE_KEY).orElse(ListTag())
                for (i in 0 until piles.size) {
                    val row = piles.getCompoundOpt(i).orElse(null) ?: continue
                    val name = row.getStringOpt(SHIP_NAME_KEY).orElse("")
        ...
        private fun readStacks(entry: CompoundTag, key: String): List<ItemStack> {
            ...
            val rows = entry.getListOpt(key).orElse(ListTag())
            for (element in rows) {
        ...
        private fun readTally(entry: CompoundTag, key: String): Map<Item, Int> {
            ...
            for (i in 0 until rows.size) {
                val row = rows.getCompoundOpt(i).orElse(null) ?: continue
                val id = row.getStringOpt(ITEM_KEY).orElse(null) ?: continue
                ...
                val item = BuiltInRegistries.ITEM.getOptional(ResourceLocation.parse(id)).orElse(null) ?: continue
