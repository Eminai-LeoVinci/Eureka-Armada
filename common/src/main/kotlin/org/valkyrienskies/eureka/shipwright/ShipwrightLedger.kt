...
class ShipwrightLedger : SavedData() {
    ...
    companion object {
        ...
        fun get(server: MinecraftServer): ShipwrightLedger {
            ...
            return server.overworld().dataStorage.computeIfAbsent({ tag -> load(tag) }, { ShipwrightLedger() }, SAVED_DATA_ID)
        }
        ...
        private fun readTally(entry: CompoundTag, key: String): Map<Item, Int> {
            ...
            for (i in 0 until rows.size) {
                ...
                val item = BuiltInRegistries.ITEM.getOptional(ResourceLocation(id)).orElse(null) ?: continue
