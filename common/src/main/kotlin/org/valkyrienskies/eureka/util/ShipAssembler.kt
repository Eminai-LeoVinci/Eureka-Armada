...
val WRECK_DISCARD: TagKey<Block> =
    TagKey.create(Registries.BLOCK, ResourceLocation(EurekaMod.MOD_ID, "wreck_discard"))
...
val WRECK_PRESERVE: TagKey<Block> =
    TagKey.create(Registries.BLOCK, ResourceLocation(EurekaMod.MOD_ID, "wreck_preserve"))
...
val WRECK_SETTLES: TagKey<Block> =
    TagKey.create(Registries.BLOCK, ResourceLocation(EurekaMod.MOD_ID, "wreck_settles"))
...
object ShipAssembler {
    ...
    fun unfillShip(
        ...
        ).filter { !it.isPassenger && it !is HangingEntity }
        ...
        val shipyardEntities = ship.shipAABB?.let { b ->
            val box = AABB(
            ...
            //
            // And BECAUSE the lookup is ship-aware, a shipyard box can answer with WORLD entities standing
            // on the hull's rendered position. Those already have correct world coordinates; putting one
            // through shipToWorld a SECOND time hurled it tens of thousands of blocks into nowhere -- that
            // was the gun seats, and their riders with them: the entire "crew nowhere to be found" report.
            // Only genuine shipyard residents take this ride.
            level.getEntities(null as Entity?, box).filter {
                VS2ChunkAllocator.isChunkInShipyardCompanion(it.blockX shr 4, it.blockZ shr 4)
            }
        } ?: emptyList()
        val fixtures = shipyardEntities.filterIsInstance<HangingEntity>()
        val fixtureAnchors = fixtures.associateWith { it.pos }

        // Everything ELSE living in the shipyard -- an armour stand placed on the deck after she sailed is the
        // one that bites. It is not a HangingEntity, so the fixture carry above could never take it, and it is
        // nowhere near the world-space riders box either: disassembly left it behind in a shipyard that goes
        // away with the ship, and the stand and everything on it went with it. Carried by the same transform
        // the riders take, since like a rider it is positioned rather than anchored to a wall.
        // Players are never in here (they ride a seat, or stand on the deck in world space); passengers move
        // with whatever carries them.
        val stowaways = shipyardEntities.filter {
        ...
        for (entity in fixtures) {
            ...
            // gridOffset, exactly as the BLOCKS take it: every block is written to floor(its world centre)
            // plus this half-block snap, and a frame anchored to the unsnapped position lands up to a block
            // off the wall it hangs on -- where survives() fails and vanilla pops it off a tick later. That
            // is the whole "frames disappear on disassembly" report; the carry was working all along.
            ...
            // Tell the CLIENTS. A vanilla item frame never moves, so its sync policy is "never send
            // position updates" (updateInterval = MAX_VALUE, no deltas) -- the server-side move above is
            // correct, but without this every client keeps its copy at the old shipyard coordinates,
            // invisible 28 million blocks away. That was the whole "frames disappear on disassembly,
            // return on reassembly" report: reassembly carried the server frame back INTO a shipyard,
            // where the ship renderer showed it again. One explicit teleport broadcast closes it.
            level.chunkSource.chunkMap.broadcast(entity, ClientboundTeleportEntityPacket(entity))
        }

        // The stowaways ride the same round trip as the riders below, but FROM shipyard space: their stored
        // position is where they stood on the deck, so one shipToWorld is the whole move.
        val alloc3 = Vector3d()
        for (entity in stowaways) {
            if (entity.isRemoved) continue
            if (wreck) { entity.discard(); continue }
            val to = shipToWorld.transformPosition(alloc3.set(entity.x, entity.y, entity.z)).add(gridOffset)
            entity.teleportTo(to.x, to.y, to.z)
            // Same client-sync insurance as the fixtures: an armour stand syncs on a slow clock, and a
            // 28-million-block jump deserves better than "eventually".
            level.chunkSource.chunkMap.broadcast(entity, ClientboundTeleportEntityPacket(entity))
        }
