...
class ShipHelmScreen(handler: ShipHelmScreenMenu, playerInventory: Inventory, text: Component) :
    ...
    private fun shipName(): String? {
        ...
        if (menu.inHand) {
            ...
            return held.takeIf { it.hasCustomHoverName() }?.hoverName?.string?.takeIf { it.isNotBlank() }
        }
        val s = ship
        if (s == null) {
    ...
    override fun mouseClicked(mouseX: Double, mouseY: Double, button: Int): Boolean {
        if (crewList.clicked(leftPos, topPos, mouseX, mouseY)) return true
    ...
    override fun renderLabels(guiGraphics: GuiGraphics, i: Int, j: Int) {
        ...
        crewList.renderClosed(guiGraphics, 0, 0, i, j, leftPos, topPos)
        ...
        menu.helmNumber?.let { number ->
            ...
            pose.pushPose()
            pose.scale(HELM_NUMBER_SCALE, HELM_NUMBER_SCALE, 1f)
     ...
     *
     * The scissor takes ABSOLUTE screen coordinates while the DRAW is panel-relative -- enableScissor
     * builds its rectangle straight from the ints it is handed and never looks at the pose (bytecode-checked
     * on 1.20.1; CrewDropdown carries the same note). Passing panel-relative bounds put the clip up at the
     * screen corner, which cut every over-long readout down to its first few characters -- present, still
     * marching along on the marquee clock, and unreadable. So the panel corner is added back here, and ONLY
     * here: the draw below stays panel-relative because the pose is already carrying that translation.
     */
    private fun drawBoxText(guiGraphics: GuiGraphics, text: String, boxX: Int, topY: Int, boxW: Int) {
        ...
        guiGraphics.enableScissor(leftPos + boxX + 2, topPos + topY, leftPos + boxX + inner, topPos + topY + BOX_H)
        pose.pushPose()
        pose.scale(scale, scale, 1f)
    ...
    override fun mouseReleased(mouseX: Double, mouseY: Double, button: Int): Boolean {
        isDragging = false
        if (getChildAt(mouseX, mouseY)
                .filter { it.mouseReleased(mouseX, mouseY, button) }.isPresent
        ) {
            return true
        }
